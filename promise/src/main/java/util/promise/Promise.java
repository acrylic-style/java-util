...
public abstract class Promise<T> implements IPromise<Object, T>, Chain<Promise<T>> {
    ...
    private final AtomicReference<PromiseStatus> status = new AtomicReference<>(PromiseStatus.PENDING);
    private Object v = null;

    private void setStatus(@NotNull PromiseStatus status) {
        if (frozen.get()) throw new IllegalStateException("Promise is frozen");
    ...
    private static <V> void setResolved(Promise<V> promise, V v) {
        PromiseStatus status = promise.status.get();
        if (status != PromiseStatus.PENDING && status != PromiseStatus.WAITING && status != PromiseStatus.RUNNING)
    ...
    private static <V> Object tryResolve(Promise<V> promise, Object o) {
        if (promise.status.get() == PromiseStatus.RESOLVED) return promise.v;
    ...
    private static Object throwIfRejected(Promise<?> promise) {
        if (promise.status.get() != PromiseStatus.REJECTED) return null;
    ...
    @Nullable
    protected static <V> Object call(Object o, Promise<V> promise, ICollectionList<Promise<?>> chain) throws Throwable {
    ...
    protected T waitUntilResolve() {
        if (this.status.get() == PromiseStatus.RESOLVED || this.status.get() == PromiseStatus.REJECTED) {
        ...
        while (true) {
            ...
            if (this.status.get() == PromiseStatus.WAITING) {
                try {
                    synchronized (this) {
                        this.wait(1);
                    }
        ...
        if (this.status.get() == PromiseStatus.PENDING) throw new IllegalStateException("Not expecting PENDING status");
        throwIfRejected(this);
        return (T) this.v;
    }
    ...
    protected T waitUntilResolve(int timeout) {
        return (T) new Watchdog(new RunnableSupplier<T>() {
            @Override
    ...
    public Promise<T> join() {
        if (this.status.get() == PromiseStatus.PENDING) this.queue();
        while (true) {
            if (Thread.interrupted()) {
                Thread.currentThread().interrupt();
                break;
            }
            if (this.status.get() == PromiseStatus.PENDING || this.status.get() == PromiseStatus.RUNNING || this.status.get() == PromiseStatus.WAITING) {
                try {
                    synchronized (this) {
                        this.wait(1);
                    }
        ...
        if (this.status.get() == PromiseStatus.PENDING || this.status.get() == PromiseStatus.WAITING || this.status.get() == PromiseStatus.RUNNING)
    ...
    public ActionableResult<T> getIfResolved() {
        if (status.get() != PromiseStatus.RESOLVED && status.get() != PromiseStatus.REJECTED)
