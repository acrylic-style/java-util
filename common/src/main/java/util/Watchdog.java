...
public class Watchdog {
    ...
    public Watchdog then(Runnable runnable) {
        if (runnable instanceof RunnableSupplier) throw new UnsupportedOperationException("This method cannot be used if you're using RunnableFunction.");
    ...
    public synchronized Object startAwait() {
        ...
        thread = new Thread(() -> {
            if (runnable instanceof RunnableSupplier) {
                try {
                    o.set(((RunnableSupplier<?>) runnable).runWithType());
                } finally {
                    terminated = true;
                    synchronized (lock) {
                        lock.notifyAll();
